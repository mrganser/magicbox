...
env.initialize(function(err, app, io, db) {  
    ...
    io.sockets.on('connection', function (socket) {
        socket.on('linkshared', function(channel, secret, link){ sharedLinksController.new(channel, secret, link, db, io); });
        socket.on('linkchanged', function(channel, secret, link){ socket.broadcast.emit('linkchanged', channel, secret, link); });
